...
public class GC10MinBenchmark {

    @Benchmark
    @BenchmarkMode(Mode.SingleShotTime)
    @OutputTimeUnit(TimeUnit.MINUTES)
    @Timeout(time = 13, timeUnit = TimeUnit.MINUTES)
    public void gcBench(Blackhole blackhole) {
        long TEN_MINUTES = 1000 * 60 * 10; // 1000ms *60s *10m
        ...
        while (System.currentTimeMillis() - start < TEN_MINUTES) {
